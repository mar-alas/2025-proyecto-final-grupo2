...
def validar_stock_disponible(productos, stock_disponible):
    ...
    for producto in productos:
        ...
        producto_en_stock = next((producto_stock for producto_stock in stock_disponible if producto_stock["id"] == producto_id), None)
        ...
        if producto_en_stock:
            ...
            if producto_en_stock["inventario_inicial"] < cantidad_solicitada:
                errores[producto_id] = {
                    "id": producto_id,
                    "nombre": producto_en_stock.get("nombre", "Nombre no disponible"),
                    "stock_disponible": producto_en_stock["inventario_inicial"],
...
def obtener_stock_disponible():
    # Validate stock availability
    url = f'http://{PRODUCTOS_HOSTNAME}:{PRODUCTOS_PORT}/api/v1/inventario/gestor_productos/productos'
    try:
